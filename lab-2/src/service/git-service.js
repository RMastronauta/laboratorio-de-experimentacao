...
const QUANTITY_PERMITED_SEARCH_REPOSITORIES_BY_REQ = 20;
  ...
  try {
    ...
    const queryGetRepositories = gql`
      query ($perPage: Int!, $after: String) {
        search(
          query: "language:Java stars:>10000"
          type: REPOSITORY
          first: $perPage
          after: $after
        ) {
...
const getRepositoriesQuantity = async (quantity) => {
  ...
  let cursorAux = null;
  console.log(`Carregando repositórios | ${quantity}`);
  ...
  progressBarStep(listRepositories.length, quantity);
  console.log(`\nCarregado todos os repositórios | ${listRepositories.length}`);
