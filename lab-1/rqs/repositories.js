...
const getRepositories = async (afterCursor) => {
  try {
    ...
    return {
      ...
      cursor:
        response.search.edges.length > 0
          ? response.search.edges[response.search.edges.length - 1].cursor
          : null,
    };
  } catch (error) {
...
const fetchAllRepositories = async () => {
  ...
  for (let index = 0; index < searchesNumber; index++) {
    const { repositories: pageRepositories, cursor } = await getRepositories(
      afterCursor
    );
